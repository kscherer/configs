        ...
        (list
         (make-mu4e-context
          ...
          :vars '((mu4e-sent-folder . "/gmail/[email].Sent Mail")
                  ...
                  (user-mail-address . "matthewzmd@gmail.com")
                  (mu4e-compose-format-flowed . t)
         ...
         (make-mu4e-context
          :name "wr"
          :enter-func (lambda () (mu4e-message "Entering context wr"))
          :leave-func (lambda () (mu4e-message "Leaving context wr"))
          :match-func
          (lambda (msg)
            (when msg
              (string-match "wr" (mu4e-message-field msg :maildir))))
          :vars '((mu4e-sent-folder . "/wr/Sent Items")
                  (mu4e-drafts-folder . "/wr/Drafts")
                  (mu4e-trash-folder . "/wr/Deleted Items")
                  (mu4e-sent-messages-behavior . sent)
                  (mu4e-compose-signature . user-full-name)
                  (user-mail-address . "matthew.zeng@windriver.com")
                  (mu4e-compose-format-flowed . t)
                  (smtpmail-queue-dir . "~/Maildir/queue/cur")
                  (message-send-mail-function . smtpmail-send-it)
                  (smtpmail-smtp-user . "mzeng") ; Set to your username
                  (smtpmail-starttls-credentials . (("prod-webmail.windriver.com" 587 nil nil)))
                  (smtpmail-auth-credentials . (expand-file-name "~/.authinfo.gpg"))
                  (smtpmail-default-smtp-server . "prod-webmail.windriver.com")
                  (smtpmail-smtp-server . "prod-webmail.windriver.com")
                  (smtpmail-smtp-service . 587)
                  (smtpmail-debug-info . t)
                  (smtpmail-debug-verbose . t)
                  (mu4e-maildir-shortcuts . ( ("/wr/INBOX"            . ?i)
                                              ("/wr/Sent Items" . ?s)
                                              ("/wr/Deleted Items"       . ?t)
                                              ("/wr/Jira"  . ?j)
                                              ("/wr/RR"  . ?l)
